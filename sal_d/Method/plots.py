...
from sal_d.Method.general import get_cuda_ifavailable
    ...
    if not os.path.exists(filename) or overwrite:
        if with_points:
            res = decoder(
                ...
                None,
                latent=latent,
                only_decoder_forward=True,
                only_encoder_forward=False,
            )
            pnts_val = res
        ...
        for key, val in z_func.items():
            ...
            layout = go.Layout(
                title=go.layout.Title(
                    text="epoch : {0} <br> input filename:{1}".format(epoch, shapefile)
                ),
                width=1200,
                height=1200,
                scene=dict(
            ...
            if with_points:
                trace_pnts = get_scatter_trace(
                    points[:, -3:], name="input", caption=caption
                )
            ...
            if save_html or not is_3d:
                offline.plot(
                    fig1, filename=filename + "_" + key + ".html", auto_open=False
                )
    ...
    for pnts in tqdm(torch.split(grid["grid_points"], 10000, dim=0)):
        ...
        if hasattr(decoder, "device_ids") and len(decoder.device_ids) > 1:
            v = decoder(
                ...
                latent=latent.repeat(len(decoder.device_ids), 1),
                only_encoder_forward=False,
                only_decoder_forward=True,
            )
        else:
...
def plot_cuts_axis(
    points, decoder, latent, path, epoch, near_zero, axis, file_name_sep="/"
):
    ...
    position_cut = [
        position_cut + i * mask.reshape(-1, 1)
        for i in np.linspace(min_axis - 0.1, max_axis + 0.1, 2)
    ]
    for index, pos in enumerate(position_cut):
        # fig = tools.make_subplots(rows=1, cols=1)

        field_input = get_cuda_ifavailable(torch.tensor(pos.T, dtype=torch.float))
        z = []
        ...
        if near_zero:
            ...
            trace1 = go.Contour(
                ...
                name="axis {0} = {1}".format(axis, pos[axis, 0]),
                autocontour=False,
        ...
        filename = "{0}{1}cutsaxis_{2}_{3}_{4}.html".format(
            path, file_name_sep, axis, epoch, index
        )
...
def plot_cuts(
    ...
    position_cut = np.vstack(([xx, np.zeros(xx.shape[0]), yy]))
    position_cut = [
        position_cut + np.array([0.0, i, 0.0]).reshape(-1, 1)
        for i in np.linspace(min_y - 0.1, max_y + 0.1, number_of_cuts)
    ]
    for index, pos in enumerate(position_cut):
        # fig = tools.make_subplots(rows=1, cols=1)
        ...
        field_input = get_cuda_ifavailable(torch.tensor(pos.T, dtype=torch.float))
        z = []
        for pnts in torch.split(field_input, 1000, dim=-1):
            input_ = pnts
            # if (not latent is None):
...
def get_grid_uniform(points, resolution, grid_boundary, is_3d):
    if is_3d:
        ...
        xx, yy, zz = np.meshgrid(x, y, z)
        grid_points = get_cuda_ifavailable(
            torch.tensor(
                np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=torch.float
            )
        )
        ...
        grid_points = get_cuda_ifavailable(
            torch.tensor(np.vstack([xx.ravel(), yy.ravel()]).T, dtype=torch.float)
        )
