...
from sal_d.Method.general import mkdir_ifnotexists
...
class GridSearch:
    def __init__(
        ...
        conf_name,
        batch_size,
        all_exps_folder="/checkpoint/matanatz/all_exps_2",
        nepochs=20000,
        constraint_gpu=False,
        gpus_per_node=1,
        partition="learnfair",
        is_continue=False,
        cluster="waic",
        ...
        mkdir_ifnotexists(all_exps_folder)

        if cluster == "fb":
            executor = submitit.AutoExecutor(folder="log_test/{0}".format(exps_folder))
            dict_params = dict(
                slurm_job_name=exps_folder + "_" + conf_name,
                timeout_min=60 * 24,
            ...
            for v1_index, v1 in enumerate(values1):
                ...
                for j in range(i + 1, len(params_list)):
                    ...
                    for v2_index, v2 in enumerate(values2):
                        str_addition = []
                        str_addition.append(
                            "{0} = {1}".format(params_list[j][0], json.dumps(v2))
                        )
                        run_name = "{0}_{1}_{2}_{3}_{4}".format(
                            conf_name,
                            params_list[i][0],
                            params_list[i][1][v1_index],
                            params_list[j][0],
                            params_list[j][1][v2_index],
                        )
                        ...
                        with open(
                            os.path.join(
                                folder, "{0}_{1}.conf".format(v1_index, v2_index)
                            ),
                            "w",
                        ) as text_file:
                            text_file.write("\n".join(str + str_addition))
                        mkdir_ifnotexists(
                            "{0}/exps_{1}".format(all_exps_folder, exps_folder)
                        )
                        runs.append(
                            dict(
                                conf=os.path.join(
    ...
    def run(self):
        if self.cluster == "fb":
            ...
            for run in self.runs:
                ...
                mkdir_ifnotexists("./exp_small/runs/")

                with open(
                    "./exp_small/runs/run_{0}.sh".format(run["run_name"]), "w"
                ) as text_file:
                    text_file.write("\n".join(template))
