...
class SALDLoss(nn.Module):
    ...
    def forward(
        ...
        recon_loss = self.l1_loss(
            network_outputs["non_mnfld_pred"].abs().squeeze(), gt_nonmnfld
        )
        ...
        if (
            "grad_on_surface" in network_outputs.keys()
            and self.grad_on_surface_weight > 0
        ):
            grad_loss = torch.min(
                torch.abs(
                    network_outputs["grad_on_surface"].squeeze() - normals_gt
                ).sum(-1),
                torch.abs(
                    network_outputs["grad_on_surface"].squeeze() + normals_gt
                ).sum(-1),
        ...
        if (
            "non_mnfld_pred_grad" in network_outputs.keys()
            and self.grad_loss_weight > 0
        ):
            grad_loss = torch.min(
                torch.abs(
                    network_outputs["non_mnfld_pred_grad"].squeeze()
                    - normals_nonmnfld_gt
                ).sum(-1),
                torch.abs(
                    network_outputs["non_mnfld_pred_grad"].squeeze()
        ...
        if "latent_reg" in network_outputs.keys() and self.latent_reg_weight > 0:
            loss = loss + self.latent_reg_weight * \
                network_outputs["latent_reg"].mean()
            debug["latent_reg_loss"] = network_outputs["latent_reg"].mean()

        if "norm_square_latent" in network_outputs.keys() and self.z_weight > 0:
            loss = loss + self.z_weight * \
                network_outputs["norm_square_latent"].mean()
